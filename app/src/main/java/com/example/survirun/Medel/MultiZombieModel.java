...
public class MultiZombieModel {
    public int markerImageResourceId;
        ...
        public void updateCurrentZombieLocation(LatLng humanLocation, Message msg) {
            ...
            MultiMapActivity.updateMarkerPos(this.currentIdx);

            /* 마지막에 거리 검사 */
            if (getZombieToHumanDistance(humanLocation) <= 5) { //단위가 몇이였더라..
                MultiMapActivity.minusHPAndCheck();
            }

            if(getZombieToHumanDistance(humanLocation) <= 300) {
                if(timeCnt >= 6) {
                    MultiMapActivity.playZBS();
                    timeCnt = 0;
                } else {
                    timeCnt++;
                }
            ...
            if(getZombieToHumanDistance(humanLocation) >= 1000) {
                thread.interrupt();
                MultiMapActivity.removeMarker(currentIdx);
                MultiMapActivity.zombieList.remove(currentIdx);
            }
        }
        ...
        public MultiZombieModel(LatLng human, int idx ,Context context) {//, int markerImageResourceId) {
            ...
            Bitmap smallMarker = Bitmap.createScaledBitmap(b, 100, 150, false);
            this.options.icon(BitmapDescriptorFactory.fromBitmap(smallMarker));
            myMarker = MultiMapActivity.mMap.addMarker(this.options);
        ...
        public Handler zombieHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                if (MultiMapActivity.isRunning) {
                    updateCurrentZombieLocation(new LatLng(MultiMapActivity.currentLat, MultiMapActivity.currentLng), msg);
                }
